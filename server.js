...
app.get('/api/users', async (req, res) => {
    ...
    try {
        const result = await client.query(`
            SELECT id, name, surname, avatar_color, avatar_image, bio, channels, status, last_seen
            FROM users
        `);
...
app.get('/api/chat-media/:chatId', async (req, res) => {
    ...
    try {
        const result = await client.query(`
            SELECT * FROM messages WHERE chat_id = $1 AND type IN ('image', 'video', 'audio', 'file')
...
app.post('/api/folders/:userId', async (req, res) => {
    ...
    try {
        await client.query(`
            INSERT INTO folders (user_id, folders)
            VALUES ($1, $2)
            ON CONFLICT (user_id) DO UPDATE SET folders = EXCLUDED.folders
...
createTables().then(() => {
    ...
    (async () => {
        ...
        try {
            ...
            if (result.rows.length === 0) {
                await client.query(`
                    INSERT INTO users (id, name, surname, password, avatar_color, avatar_image, bio, status)
                    VALUES ('@support', 'Поддержка', 'Telepam', 'support123', '#0088cc', NULL, 'Официальная поддержка Telepam', 'online')
                `);
                await client.query(`
                    INSERT INTO settings (user_id, privacy, blocked, notifications, sound)
                    VALUES ('@support', '{"showBio": true, "showChannels": true, "whoCanMessage": "all"}', '{}', true, true)
                `);
