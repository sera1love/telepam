...
async function createTables() {
    ...
    try {
        await client.query(`CREATE TABLE IF NOT EXISTS users (
            ...
            public_key TEXT, channels JSONB DEFAULT '[]', status TEXT DEFAULT 'offline',
            last_seen TIMESTAMP DEFAULT CURRENT_TIMESTAMP, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )`);
...
// API
app.get('/api/public-key', (req, res) => res.json({ publicKey: 'server-key' }));
...
app.get('/api/users', async (req, res) => {
    ...
    try {
        ...
        res.json(result.rows.map(u => ({ ...u, avatarColor: u.avatar_color, avatarImage: u.avatar_image, online: onlineUsers.has(u.id) })));
    } catch (err) { console.error('Users error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/user/:id', async (req, res) => {
    ...
    } catch (err) { console.error('User error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/friends/:userId', async (req, res) => {
    ...
    } catch (err) { console.error('Friends error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/groups', async (req, res) => {
    const client = await pool.connect();
    try { const result = await client.query('SELECT * FROM groups'); res.json(result.rows); }
    catch (err) { console.error('Groups error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.post('/api/groups', async (req, res) => {
    ...
    } catch (err) { console.error('Create group error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/chats/:userId', async (req, res) => {
    ...
    try {
        ...
        res.json(chats);
    } catch (err) { console.error('Chats error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/messages/:chatId', async (req, res) => {
    ...
    } catch (err) { console.error('Messages error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.get('/api/chat-media/:chatId', async (req, res) => {
    ...
    try {
        ...
        res.json(media);
    } catch (err) { console.error('Media error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.delete('/api/messages/:chatId/:index', async (req, res) => {
    ...
    } catch (err) { console.error('Delete error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});

app.get('/api/settings/:userId', async (req, res) => {
    const client = await pool.connect();
    try { const result = await client.query('SELECT * FROM settings WHERE user_id = $1', [req.params.userId]); res.json(result.rows[0] || {}); }
...
app.post('/api/block/:userId', async (req, res) => {
    ...
    try {
        ...
        if (!blocked.includes(blockedId)) { blocked.push(blockedId); await client.query('UPDATE settings SET blocked = $2 WHERE user_id = $1', [req.params.userId, blocked]); }
        res.json({ success: true });
    } catch (err) { console.error('Block error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
...
app.delete('/api/block/:userId/:blockedId', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true });
    } catch (err) { console.error('Unblock error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});

app.get('/api/folders/:userId', async (req, res) => {
    const client = await pool.connect();
    try { const result = await client.query('SELECT folders FROM folders WHERE user_id = $1', [req.params.userId]); res.json(result.rows[0]?.folders || [{ id: 'all', name: 'Все чаты', chats: [], icon: '💬' }]); }
    catch (err) { console.error('Folders error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});

app.post('/api/folders/:userId', async (req, res) => {
    const client = await pool.connect();
    try { await client.query(`INSERT INTO folders (user_id, folders) VALUES ($1, $2) ON CONFLICT (user_id) DO UPDATE SET folders = EXCLUDED.folders`, [req.params.userId, JSON.stringify(req.body)]); res.json({ success: true }); }
    catch (err) { console.error('Folders error:', err); res.status(500).json({ error: 'Server error' }); }
    finally { client.release(); }
});
